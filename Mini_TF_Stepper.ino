...
int k;
int buttonApin = 4;  // the push switch is connected on pin 4
...
void setup()
{
  ...
  pinMode(buttonApin, INPUT_PULLUP);  // set the button to default value HIGH for the input, not pushed
}

void loop()
{
  
  while (digitalRead(buttonApin) != LOW)  // initialize the angle to 0 degree when the button is pushed
  {
  ...
  while (j<180) // allows to send the data for each angle from 0 degrees to 179 degrees
    {
  ...
  while (digitalRead(buttonApin) != LOW) // allows to send the data for each angle from 180 degrees to 1 degrees
    {
      myStepper.step(-6); // move the stepmotor by 1 degree in the other direction 
      somme = 0; // reset the sum value
